...
SIZE = 0.02
ENGINE_POWER            = 15000000*SIZE*SIZE
WHEEL_MOMENT_OF_INERTIA = 5000*SIZE*SIZE
WHEEL_R  = 27
WHEEL_W  = 14
WHEELPOS = [(-60, +47), (+60,+47), (-60,-47), (+60,-47)]
AGENT_POLY = [(-50, 75), (-50, -75), (50, 75), (50, -75)]
WHEEL_COLOR = (0.0,0.0,0.0)
WHEEL_WHITE = (0.1,0.1,0.1)
MUD_COLOR   = (0.4,0.4,0.0)
# friction ~= mass ~= size^2 (calculated implicitly using density)

class Car:
    def __init__(self, world, init_angle, init_x, init_y, friction_limit=1200000):
        self.world = world
        self.friction_limit = friction_limit*SIZE*SIZE
        self.hull = self.world.CreateDynamicBody(
            position=(init_x, init_y),
            angle=init_angle,
            fixtures=fixtureDef(
                shape=polygonShape(vertices=[ (x*SIZE, y*SIZE) for x, y in AGENT_POLY]),
                density=7.5,
                friction=0.1,
                groupIndex=1,
                categoryBits=0x0010,
    ...
    def step(self, dt, brake_force=15):
        for w in self.wheels:
            # Steer each wheel
            ...
            for tile in w.tiles:
                friction_limit = max(friction_limit, self.friction_limit*tile.road_friction)
                grass = False
            ...
            if w.brake >= 0.9:
                ...
                BRAKE_FORCE = brake_force    # radians per second
                dir = -np.sign(w.omega)
